...
class Player:
    ...
    def getCellAhead(self, cell, direction):
        ...
        """ If possible, this function returns the cell next to 'cell' in
            ...
            This function is also called to get the cells to the left and right of
            the current cell, to be able to draw 'left_edge' and 'right_edge' of a
            cell. So also the direction has to be a separated function parameter.
    ...
    def checkThisAndNextCellForWallToSide(self, cell, left_or_right):
        otherdirection = cell.getRotate(self.direction, left_or_right)
        if cell.has_wall(otherdirection):
            return True
        nextcell = self.getCellAhead(cell, self.direction)
        if not nextcell:
            return True
        if cell.has_wall(otherdirection):
            return True
...
class DrawScreen:
    ...
    def drawCells(self):
        ...
        for cell in cells:
            ...
            self.drawverticals["left"] = self.player.checkThisAndNextCellForWallToSide(cell, "left")
            self.drawverticals["right"] = self.player.checkThisAndNextCellForWallToSide(cell, "right")
    ...
    def drawShape(self, shape, drawingsize):
        ...
        self.drawVerticalLines(drawingsize)

        if shape == "center_wall":
            self.drawLine(self.centerx - drawingsize, self.centery - drawingsize,
                          self.centerx + drawingsize, self.centery - drawingsize)
            self.drawLine(self.centerx - drawingsize, self.centery + drawingsize,
                          self.centerx + drawingsize, self.centery + drawingsize)
            return

        if shape == "left_wall":
            self.drawLine(self.centerx - oldsize, self.centery - oldsize,
                          self.centerx - drawingsize, self.centery - drawingsize)
            self.drawLine(self.centerx - oldsize, self.centery + oldsize,
                          self.centerx - drawingsize, self.centery + drawingsize)
            return
        ...
        if shape == "left_edge":
            self.drawLine(self.centerx - oldsize, self.centery - drawingsize,
                          self.centerx - drawingsize, self.centery - drawingsize)
            self.drawLine(self.centerx - oldsize, self.centery + drawingsize,
                          self.centerx - drawingsize, self.centery + drawingsize)
            return
    ...
    def drawVerticalLines(self, drawingsize):

        if self.drawverticals["left"]:
            # Vertical lines shared across all shapes (closes the shapes)
            self.drawLine(self.centerx - drawingsize, self.centery - drawingsize,
                          self.centerx - drawingsize, self.centery + drawingsize)

        if self.drawverticals["right"]:
            self.drawLine(self.centerx + drawingsize, self.centery - drawingsize,
                          self.centerx + drawingsize, self.centery + drawingsize)

    def drawLine(self, from_x, from_y, to_x, to_y):

        pygame.draw.line(self.surface, COLORS["white"],
                         (from_x * SCALEFACTOR, from_y * SCALEFACTOR),
                         (to_x * SCALEFACTOR, to_y * SCALEFACTOR),
                         self.linesize)
